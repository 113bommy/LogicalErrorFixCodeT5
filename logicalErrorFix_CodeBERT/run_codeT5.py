    ...
    if args.do_train:
        ...
        for step in bar:
            ...
            if args.do_eval and ((global_step + 1) %args.eval_steps == 0) and eval_flag:
                ...
                last_output_dir = os.path.join(args.output_dir, 'checkpoint-last-CodeT5')
                ...
                if eval_loss<best_loss:
                    ...
                    output_dir = os.path.join(args.output_dir, 'checkpoint-best-ppl-CodeT5')
                ...
                if args.model_name_or_path != "Salesforce/codet5-base":
                    ...
                    with open(os.path.join(args.output_dir,"dev-CodeT5.output"),'w') as f, open(os.path.join(args.output_dir,"dev-CodeT5.gold"),'w') as f1:
                        ...
                        for example_index, (ref,gold) in enumerate(zip(p,eval_code)):
                            predictions.append(str(example_index)+'\t'+ref)
                            f.write(str(example_index)+'\t'+ref+'\n')
                            f1.write(str(example_index)+'\t'+gold[example_index]["target"]+'\n')
                ...
                if args.model_name_or_path != "Salesforce/codet5-base":
                    ...
                    if dev_bleu>best_bleu:
                        ...
                        best_bleu=dev_bleu
                        # Save best checkpoint for best bleu
                        output_dir = os.path.join(args.output_dir, 'checkpoint-best-bleu-CodeT5')
                        if not os.path.exists(output_dir):
                            os.makedirs(output_dir)
                        model_to_save = model.module if hasattr(model, 'module') else model  # Only save the model it-self
                        output_model_file = os.path.join(output_dir, "pytorch_model.bin")
