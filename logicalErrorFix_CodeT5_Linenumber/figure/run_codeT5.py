...
def main():
    ...
    # config_class - not used in CodeT5-base model
    config_class, model_class, tokenizer_class = MODEL_CLASSES[args.model_type]
    if config_class is not None:
        config = config_class.from_pretrained(args.config_name if args.config_name else args.model_name_or_path)

    # tokenizer - used same as CodeBert
    ...
    if args.do_train:
        ...
        for step in bar:
            ...
            if args.do_eval and ((global_step + 1) %args.eval_steps == 0) and eval_flag:
                ...
                last_output_dir = os.path.join(args.output_dir, 'checkpoint-last-CodeT5-Line-Number')
                ...
                if eval_loss<best_loss:
                    ...
                    output_dir = os.path.join(args.output_dir, 'checkpoint-best-ppl-CodeT5-Line-Number')
                ...
                if args.model_name_or_path != "Salesforce/codet5-base":
                    ...
                    with open(os.path.join(args.output_dir,"dev-CodeT5-Line-Number.output"),'w') as f, open(os.path.join(args.output_dir,"dev-CodeT5-Line-Number.gold"),'w') as f1:
                        for ref,gold in zip(p,eval_examples):
                            predictions.append(str(gold.idx)+'\t'+ref)
                            f.write(str(gold.idx)+'\t'+ref+'\n')
                            f1.write(str(gold.idx)+'\t'+gold.target+'\n')
                ...
                if args.model_name_or_path != "Salesforce/codet5-base":
                    ...
                    (goldMap, predictionMap) = bleu.computeMaps(predictions, os.path.join(args.output_dir, "dev-CodeT5-Line-Number.gold")) 
                    dev_bleu=round(bleu.bleuFromMaps(goldMap, predictionMap)[0],2)
                    logger.info("  %s = %s "%("bleu-4",str(dev_bleu)))
                    logger.info("  "+"*"*20)    
                    if dev_bleu>best_bleu:
                        ...
                        best_bleu=dev_bleu
                        # Save best checkpoint for best bleu
                        output_dir = os.path.join(args.output_dir, 'checkpoint-best-bleu-CodeT5-Line-Number')
                        if not os.path.exists(output_dir):
                            os.makedirs(output_dir)
                        model_to_save = model.module if hasattr(model, 'module') else model  # Only save the model it-self
                        output_model_file = os.path.join(output_dir, "pytorch_model.bin")
    ...
    if args.do_test:
        ...
        for idx,file in enumerate(files):   
            ...
            if args.model_name_or_path != "Salesforce/codet5-base":
                with open(os.path.join(args.output_dir + '/' + modelName, fileName + "_with_line_{}.output".format(str(idx))),'w') as f, open(os.path.join(args.output_dir + '/' + modelName, fileName + "_with_line_{}.gold".format(str(idx))),'w') as f1:
                    for ref,gold in zip(p,eval_examples):
                        predictions.append(str(gold.idx)+'\t'+ref)
                        f.write(str(gold.idx)+'\t'+ref+'\n')
                        f1.write(str(gold.idx)+'\t'+gold.target+'\n')
            ...
            (goldMap, predictionMap) = bleu.computeMaps(predictions, os.path.join(args.output_dir + '/' + modelName, fileName + "_with_line_{}.gold".format(idx)))
            dev_bleu=round(bleu.bleuFromMaps(goldMap, predictionMap)[0],2)
            logger.info("  %s = %s "%("bleu-4",str(dev_bleu)))
            logger.info("  "+"*"*20)    
